...
Create a table where you summarise monthly returns for each of the stocks and `SPY`; min, max, median, mean, SD, and variance `var`. Also, using the `PerformanceAnalytics` package, calculate the skenwness and kurtosis, using `PerformanceAnalytics::skeweness()` and `PerformanceAnalytics::kurtosis()` 
...
> You may find useful the material on [downloading economic data from the FRED](https://mfa2023.netlify.app/reference/finance_data/#data-from-the-federal-reserve-economic-data-with-tidyquant). 
...
knitr::include_graphics(here::here("images", "german_electins_polls.png"), error = FALSE)
```
...
german_election_polls <- polls[[3]] %>% # the first table on the page contains the list of all opinions polls
  slice(3:(n()-1)) %>%  # drop the first and second row, as it contains variable names and actual result, and last row that contains 2017 results
  mutate(
...
**Please seek out help when you need it!**  You know enough R (and have enough examples of code from class and your readings) to be able to do this. If you get stuck, ask for help from others, post a question on Slack-- and remember that I am here to help too!  
