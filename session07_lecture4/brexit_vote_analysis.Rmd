...
# to check model performance + diagnostics
library(performance)
...
mosaic::msummary(leave_educ_age)
```
...
leave_politics <- lm(leave_share ~ con_2015 + lab_2015 + ld_2015 + ukip_2015, 
                     data = brexit_results)
mosaic::msummary((leave_politics))
```
...
mosaic::msummary(leave_everything)
```
...
performance::check_model(leave_everything)
...
mosaic::msummary(leave_everything2)
vif(leave_everything2)
performance::check_model(leave_everything2)
